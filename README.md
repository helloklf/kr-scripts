...
- 输入参数如 `KrScriptCore.executeShellAsync([要执行的脚本], [日志回调函数名])`
- 返回是否调用成功（只表示是否成功启动进程，不表示执行代码时是否出现错误）
- 例如：
...
> 暂不支持 `参数`
- `executeShellAsync`还未添加`参数`定义功能
- 因此【`switch`中`setstate`】及【`action`中`script`】的脚本还不能立即迁移到网页中调用
- 为了保持脚本格式的一致性，后续的更新会完善这一点~
